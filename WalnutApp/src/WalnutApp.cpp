...
static int m_AddTask=0, m_RemoveTask=0, m_MarkTaskAsComplete=0, m_Level=1,m_Hour=23,m_Minute=59,m_year=1999,m_CreateTask=0,m_Time;
...
static int month_current = 1;
static int day_current = 1;
int counter = 0;
...
public:
	...
	virtual void OnUIRender() override
	{
		...
		{
			ImGui::NewLine();
			ImGui::InputTextWithHint("-_-", "Enter Task Name", m_task, IM_ARRAYSIZE(m_task));
			ImGui::NewLine();
			ImGui::InputInt("Enter Priority Level of the Task(1-3)", &m_Level);
			ImGui::NewLine();
			ImGui::InputInt(":", &m_Hour); ImGui::InputInt("Enter Time(in 24 hour format)", &m_Minute);
			ImGui::NewLine();
					...
					for (int column = 0; column < 3; column++)
					{
						if (past != NULL && column == 0 && !retToDo(past->a))
						{
                         ...
                         //   name = "McD";
                           ...
                           if(ImGui::Checkbox(" ", &checkRT))
						editStatus(exact);
							
							ImGui::SameLine(); if (checkRT) { ImGui::Text("Completed"); }
							else { ImGui::Text("Incomplete"); }
                            ImGui::SameLine(); ImGui::Text("\t\t\t\t\t\t\t\t\t\t"); ImGui::SameLine(); if (ImGui::Button("Delete")) { deletion(exact); ll.delete_node(exact); };
							ImGui::Text("%d/%d/%d      %d", retDate(exact), retMonth(exact),retYear(exact), retTime(exact));
                            past = past->rlink;
						...
						else if(present != NULL && column == 1)
                        {
                            ...
                            ImGui::TableSetColumnIndex(column);
                            ImGui::Text("%s", name.c_str());
                            bool check = retStatus(exact);
                            if (ImGui::Checkbox(" ", &check))
                            {
                                editStatus(exact);
                            }
                            ImGui::SameLine(); if (check) { ImGui::Text("Completed"); }
                            else { ImGui::Text("Incomplete"); }
                            ImGui::SameLine(); ImGui::Text("\t\t\t\t\t\t\t\t\t\t"); ImGui::SameLine(); if (ImGui::Button("Delete")) { deletion(exact); ll.delete_node(exact); };
                            ImGui::Text("%d/%d/%d      %d", retDate(exact), retMonth(exact), retYear(exact), retTime(exact));
